...
export default function App() {
  ...
  const syncMuted = (m: boolean) => { isMutedRef.current = m; setIsMuted(m) }
  ...
  const handleSpeechEnd = useCallback(async (audioFloat32: Float32Array) => {
    if (isMutedRef.current) return
    const currentPhase = phaseRef.current
    if (currentPhase !== 'listening' && currentPhase !== 'recommendations') return
  ...
  }, [detectCity, startVAD, pauseVAD, speak, resumeVAD, resetHistory])
  ...
  const handleMute = useCallback(() => {
    ...
    pauseVAD()
    stopTTS()
    if (['listening', 'recommendations', 'speaking'].includes(phaseRef.current)) {
  ...
  // ---------- card selection ----------
  const handleCardSelect = useCallback(async (idx: number) => {
    ...
    try {
      const { spokenText } = await sendMessage(`I'd like to go to ${picked.name}.`)
      speak(spokenText, () => {})
    } catch {
  ...
  return (
    <div className="app-root">
      <header className="app-header">
        ...
        {isSessionActive && (
          <button className="app-reset-btn" onClick={handleStart} aria-label="Start over">
            New Search
          </button>
        )}
      </header>
      ...
      <main className="app-main">
        <section className="app-hero">
          {phase === 'idle' && (
            <div className="app-intro">
              <h1 className="app-headline">Find your next<br />great meal.</h1>
              <p className="app-sub">Tell me what you're craving — I'll find it.</p>
            </div>
          )}
        ...
        {restaurants.length > 0 && (
          <section className="app-cards-section" aria-label="Restaurant recommendations">
